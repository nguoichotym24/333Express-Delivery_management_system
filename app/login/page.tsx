    ...
    <div className="min-h-screen flex items-center justify-end p-4 md:p-8" style={{ backgroundImage: "url('/bg-loginsignup.png')", backgroundSize: "cover", backgroundPosition: "center", backgroundAttachment: "fixed" }}>
      <div className="w-full max-w-md bg-white rounded-lg shadow-2xl p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <div className="flex items-center border-b-2 border-gray-300 focus-within:border-primary transition-colors">
              ...
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="SĐT / Tên người dùng / Email" className="flex-1 bg-transparent py-3 text-gray-900 placeholder-gray-400 focus:outline-none" required />
            </div>
          </div>
          <div>
            <div className="flex items-center border-b-2 border-gray-300 focus-within:border-primary transition-colors">
              ...
              <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Mật khẩu" className="flex-1 bg-transparent py-3 text-gray-900 placeholder-gray-400 focus:outline-none" required />
              ...
              <Link href="/forgot-password" className="text-primary hover:underline text-sm ml-2 whitespace-nowrap">Quên?</Link>
            </div>
          </div>
          <Button type="submit" disabled={loading} className="w-full bg-primary text-white hover:bg-primary/90 py-3 font-medium rounded-lg transition-colors">{loading ? "Đang đăng nhập..." : "Đăng nhập"}</Button>
        </form>
        ...
        <p className="text-xs text-gray-600 text-center mb-6">
          Bằng việc tiếp tục, bạn đồng ý với <Link href="/terms" className="text-primary hover:underline">Điều khoản dịch vụ</Link> và <Link href="/privacy" className="text-primary hover:underline">Chính sách bảo mật</Link>.
        </p>

        <div className="text-center text-sm text-gray-600">
          <p> Bạn chưa có tài khoản? <Link href="/signup" className="text-primary hover:underline font-semibold">Đăng ký</Link></p>
        </div>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          ...
          <div className="space-y-2">
            {demoAccounts.map((account) => (
              <button key={account.email} type="button" onClick={() => handleDemoAccount(account.email)} className="w-full text-left px-3 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition-colors text-sm">
