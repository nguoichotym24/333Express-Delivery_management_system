...
export default function OrdersPage() {
  ...
  const filteredOrders =
    filterStatus === "all" ? orders.orders : orders.orders.filter((o: any) => o.status === filterStatus)
    ...
    <DashboardLayout>
      <div className="space-y-8">
        ...
        <div>
          ...
          <p className="text-secondary">Quản lý tất cả đơn hàng của bạn</p>
        </div>

        {/* Filters */}
        <div className="bg-surface border border-default rounded-xl p-6">
          <div className="flex flex-wrap gap-2">
            {["all", "order_created", "picked_up", "at_warehouse", "in_transit", "delivered"].map((status) => (
              <button
                key={status}
              ...
              <tbody>
                {filteredOrders.map((order: any) => (
                  <tr key={order.id} className="border-b border-default hover:bg-background transition-colors">
                    ...
                    <td className="px-6 py-4 text-sm text-secondary">{order.receiverInfo.address}</td>
